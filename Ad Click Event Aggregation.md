...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-001.svg" width="700"/>
  <p>Figure 1 RTB process</p>
</div>
...
**Interviewer**: It’s a log file located in different servers and the latest click events are appended to the end of the log file. The event has the following attributes:*ad\_id, click\_timestamp, user\_id, ip,*and*country*.
...
- Support data filtering by*ip*,*user\_id,*or*country*for the above two queries.
...
- Return the top 100 most clicked*ad\_id*every minute.
...
- Aggregate the number of clicks of*ad\_id*in the last*M*minutes.
- Return the top*N*most clicked*ad\_ids*in the last*M*minute.
- Support aggregation filtering by different attributes.
...
| **API** | **Detail** |
| --- | --- |
| GET /v1/ads/popular\_ads | Return top*N*most clicked ads in the last*M*minutes |
...
Table 5 Request parameters for /v1/ads/popular\_ads
...
To support ad filtering, we add an additional field called*filter\_id*to the table. Records with the same*ad\_id*and*click\_minute*are grouped by*filter\_id*as shown in Table 9, and filters are defined in Table 10.
...
| --- | --- | --- |
| **most\_clicked\_ads** |  |  |
| window\_size | integer | The aggregation window size (*M*) in minutes |
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-002.svg" width="700"/>
  <p>Figure 2 Aggregation workflow</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-003.svg" width="700"/>
  <p>Figure 3 High-level design</p>
</div>

What is stored in the first message queue? It contains ad click event data as shown in Table 13.

| **ad\_id** | **click\_timestamp** | **user\_id** | **ip** | **country** |
| --- | --- | --- | --- | --- |
...
1. Top *N* most clicked ads aggregated at per-minute granularity.

| update\_time\_minute | most\_clicked\_ads |
| --- | --- |
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-004.svg" width="700"/>
  <p>Figure 4: End-to-end exactly once</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-005.svg" width="600"/>
  <p>Figure 5 Aggregation service</p>
</div>
...
A Map node reads data from a data source, and then filters and transforms the data. For example, a Map node sends ads with*ad\_id % 2 = 0*to node 1, and the other ads go to node 2, as shown in Figure 6.

<div align="center">
  <img src="images/ad-click-event-aggregation/image-006.svg" width="500"/>
  <p>Figure 6 Map operation</p>
</div>

You might be wondering why we need the Map node. An alternative option is to set up Kafka partitions or tags and let the aggregate nodes subscribe to Kafka directly. This works, but the input data may need to be cleaned or normalized, and these operations can be done by the Map node. Another reason is that we may not have control over how data is produced and therefore events with the same*ad\_id*might land in different Kafka partitions.

**Aggregate node**

An Aggregate node counts ad click events by*ad\_id*in memory every minute. In the MapReduce paradigm, the Aggregate node is part of the Reduce. So the map-aggregate-reduce process really means map-reduce-reduce.
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-007.svg" width="700"/>
  <p>Figure 7 Reduce node</p>
</div>
...
As shown in Figure 8, input events are partitioned by ad\_id (ad\_id % 3) in Map nodes and are then aggregated by Aggregation nodes.

<div align="center">
  <img src="images/ad-click-event-aggregation/image-008.svg" width="700"/>
  <p>Figure 8 Aggregate the number of clicks</p>
</div>

##### Use case 2: return top*N*most clicked ads

Figure 9 shows a simplified design of getting the top 3 most clicked ads, which can be extended to top*N*. Input events are mapped using ad\_id and each Aggregate node maintains a heap data structure to get the top 3 ads within the node efficiently. In the last step, the Reduce node reduces 9 ads (top 3 from each aggregate node) to the top 3 most clicked ads every minute.

<div align="center">
  <img src="images/ad-click-event-aggregation/image-009.svg" width="700"/>
  <p>Figure 9 Return top N most clicked ads</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-010.svg" width="700"/>
  <p>Figure 10 Lambda and Kappa architectures</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-011.svg" width="700"/>
  <p>Figure 11 Recalculation service</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-012.svg" width="700"/>
  <p>Figure 12 Late events</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-013.svg" width="700"/>
  <p>Figure 13 Miss events in an aggregation window</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-014.svg" width="700"/>
  <p>Figure 14 Watermark</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-015.svg" width="700"/>
  <p>Figure 15 Tumbling window</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-016.svg" width="700"/>
  <p>Figure 16 Sliding window</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-017.svg" width="600"/>
  <p>Figure 17 Duplicate data</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-018.svg" width="600"/>
  <p>Figure 18 Record the offset</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-019.svg" width="600"/>
  <p>Figure 19 Save offset after receiving ack</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-020.svg" width="600"/>
  <p>Figure 20 Distributed transaction</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-021.svg" width="600"/>
  <p>Figure 21 Add consumers</p>
</div>
...
- **Hashing key** Using*ad\_id*as hashing key for Kafka partition to store events from the same*ad\_id*in the same Kafka partition. In this case, an aggregation service can subscribe to all events of the same*ad\_id*from one single partition.
- **The number of partitions** If the number of partitions changes, events of the same*ad\_id*might be mapped to a different partition. Therefore, it’s recommended to pre-allocate enough partitions in advance, to avoid dynamically increasing the number of partitions in production.
- **Topic physical sharding** One single topic is usually not enough. We can split the data by geography (topic\_north\_america, topic\_europe, topic\_asia, etc.,) or by business type (topic\_web\_ads, topic\_mobile\_ads, etc).
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-022.svg" width="700"/>
  <p>Figure 22 Aggregation service</p>
</div>
...
Option 1: Allocate events with different ad\_ids to different threads, as shown in Figure 23.

<div align="center">
  <img src="images/ad-click-event-aggregation/image-023.svg" width="600"/>
  <p>Figure 23 Multi-threading</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-024.png" width="600"/>
  <p>Figure 24 Virtual nodes [22]</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-025.svg" width="700"/>
  <p>Figure 25 Allocate more aggregation nodes</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-026.svg" width="700"/>
  <p>Figure 26 Data in a snapshot</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-027.svg" width="600"/>
  <p>Figure 27 Aggregation node failover</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-028.svg" width="700"/>
  <p>Figure 28 Final design</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-029.svg" width="700"/>
  <p>Figure 29 Alternative design</p>
</div>
...
<div align="center">
  <img src="images/ad-click-event-aggregation/image-030.png" width="600"/>
</div>
